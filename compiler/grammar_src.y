...
%token NAME COLON SEMICOLON RIGHT_ARROW LEFT_CURLY_BRACE RIGHT_CURLY_BRACE LEFT_PARENTHESIS RIGHT_PARENTHESIS SINGLECOMMENT MULTILINECOMMENTS PUTS QUOTES NUMBER SET INTEGER_VALUE DOLLAR_SIGN INC DEC LOGICAL TRUE FALSE NTOL CHARACTERS GETS STDIN PLUS MINUS MUL DIV EQ GT LE GE LT NE LEFT_BRACKET RIGHT_BRACKET QUESTION_MARK EXCLAMATION_MARK
%start input
...
tertiaryOperator:
	LEFT_BRACKET logicalComparation RIGHT_BRACKET QUESTION_MARK statement EXCLAMATION_MARK statement{
			//$$ = "cout" << \"now working\";";
			$$ = "if (" + $2 + "){" + $5 + "} else {" + $7 + "}\n";		
	}
	;
